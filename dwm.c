...
static void verttile(Monitor *m);
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (ev->x > selmon->ww - (int)TEXTW(stext) + lrpad - 2)
			click = ClkStatusText;
		else
			click = ClkWinTitle;
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	if (c->isfloating || c->mon->lt[c->mon->sellt]->arrange == NULL) {
		...
		if ((c->mon->lt[c->mon->sellt]->arrange == monocle || n == 1) && smartgaps == 1) {
			gapoffset = 0;
			gapincr = -2 * borderpx;
			wc.border_width = 0;
		} else {
...
col(Monitor *m)
{
	...
	for (i = x = y = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->rmaster ? x + m->wx + m->ww - mw : x + m->wx, m->wy, w - (2 * c->bw), m->wh - (2 * c->bw), 0);
			x += WIDTH(c);
		} else {
			h = (m->wh - y) / (n - i);
			resize(c, m->rmaster ? m->wx : m->wx + x, m->wy + y,
...
tile(Monitor *m)
{
	...
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			h = (m->wh - my) / (MIN(n, m->nmaster) - i);
			resize(c, m->rmaster ? m->wx + m->ww - mw - (n > m->nmaster ? gappx : 0) : m->wx + (n > m->nmaster ? 0 : (m->ww-mw)/2),
			       m->wy + my, mw - (2*c->bw) + (n > m->nmaster ? gappx : 0), h - (2*c->bw), 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c);
		} else {
